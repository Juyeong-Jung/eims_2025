...
public class EmployeeController {
	...
	public String logicalDeleteEmployee(EmployeeForm employeeForm, Model model) {
		// サービス層で削除処理を実行
		Employee employee = employeeService.findByEmployee(employeeForm.getEmpno());
		employeeService.logicalDeleteByEmployee(employee);
	...
	/** 退職者リスト表示 */
	// 退職者一覧を表示（select.html の「退職者閲覧」から遷移）
	@GetMapping("/retired")
	public String showRetiredEmployees(Model model) {
		model.addAttribute("employees", employeeService.findRetiredEmployees());
		return "search_result_retired";
	}
	
	/** 社員を物理削除する */
	@PostMapping("/physicalDeleteEmployee/{empno}")
	public String deleteEmployee(@PathVariable Integer empno, Model model) {
		// サービス層で削除処理を実行
	...
	/** 退職社員を復元する */
	@PostMapping("/restoreDeleteEmployee/{empno}")
	public String restoreDeleteEmployee(@PathVariable Integer empno, Model model) {
		Employee employee = employeeService.findByEmployee(empno);
